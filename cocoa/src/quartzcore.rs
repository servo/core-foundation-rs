...
impl CALayer {
    ...
    pub fn sublayers(&self) -> Vec<CALayer> {
        unsafe {
            ...
            let sublayers: CFArray = TCFType::wrap_under_create_rule(sublayers);
            sublayers.into_iter().map(|elem| CALayer((*elem) as id)).collect()
        }
    }
    ...
    pub fn set_sublayer_transform(&self, sublayer_transform: CATransform3D) {
        msg_send![self.id(), sublayer_transform]
    }
...
pub mod transaction {
    ...
    pub fn set_value_for_key(value: id, key: &str) {
        let key: CFString = CFString::from(key);
        msg_send![class!(CATransaction), setValueForKey:key]
    }
}
